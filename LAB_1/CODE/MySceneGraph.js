...
class MySceneGraph {
    ...
    parseMaterials(materialsNodes){
        ...
        for (let material of materialNodes) {
            ...
            var enabledVal = this.reader.getBoolean(material, 'shininess');

            var ambientVal = this.getRGBAElement(omni.getElementsByTagName('emission')[0]);
            var diffuseVal = this.getRGBAElement(omni.getElementsByTagName('ambient')[0]);
            var specularVal = this.getRGBAElement(omni.getElementsByTagName('diffuse')[0]);
            var specularVal = this.getRGBAElement(omni.getElementsByTagName('specular')[0]);
...
class MySceneGraph {
    ...
    parseXMLFile(rootElement) {
       ...
       //<material>
       if ((index = nodeNames.indexOf("material")) == -1)
       return "tag <material> missing";
        else {
            if (index != MATERIAL_INDEX)
                this.onXMLMinorError("tag <material> out of order");
            if ((error = this.parseMaterial(nodes[index])) != null)
                return error;
        }
